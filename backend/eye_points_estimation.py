...
predictor_path="data/params/shape_predictor_68_face_landmarks.dat"
...
fid = cv2.FileStorage('data/params/cameraCalib.xml', cv2.FileStorage_READ)
...
def estimateHeadPose(landmarks:np.ndarray, face_model:np.ndarray, iterate=True):
    """
    顔のランドマークと顔モデルを使用して、顔の姿勢・位置座標(カメラを基準とした３D座標系上)を推定します。
    Args:
        landmarks: 顔のランドマーク座標（2D）
        face_model: 一般的な顔モデルの相対的な位置座標（3D）
        camera: カメラ行列
        distortion: カメラの歪み係数
    ...
    Returns:
        ...
        tvec: 3D位置ベクトル
    """
    _, rvec, tvec = cv2.solvePnP(face_model, landmarks, camera_matrix, camera_distortion, flags=cv2.SOLVEPNP_EPNP)
    if iterate:
        _, rvec, tvec = cv2.solvePnP(face_model, landmarks, camera_matrix, camera_distortion, rvec, tvec, True)
    print("rvec:",rvec)
    print("tvec:",tvec)

    return rvec, tvec

def estimate_eye_location(face:np.ndarray, hr:np.ndarray, ht:np.ndarray)->Tuple[np.ndarray,np.ndarray]:
    """
...
def fetch_eyes(path:str)->Tuple[np.ndarray,np.ndarray]:
    ...
    landmarks, general_landmarks = get_facial_landmarks(img)
    #グローバル(S接近前)座標での、それぞれの顔座標を定義
    face = np.loadtxt('data/params/faceModelGeneric.txt')
    num_pts = face.shape[1]
    facePts = face.T.reshape(num_pts, 1, 3)
    landmarks = landmarks.astype(np.float32)
    landmarks = landmarks.reshape(num_pts, 1, 2)
    hr, ht = estimateHeadPose(landmarks, facePts)#カメラの位置をグローバル(S接近前)→ローカルにする際のhr,htを算出

    local_right_eye_center,local_left_eye_center = estimate_eye_location(face, hr, ht)#グローバルカメラから見たときの[img_warped,hr_norm,gc_normalized](gc_normalizedはgc参照で算出したものなのであてにならない)
